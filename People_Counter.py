...
def save_log(direction, now):
    ...
    try:
        ...
        log_file.write(f"{now.strftime('%d.%m.%Y')}|{ms_now}|{direct[0]}|{direct[1]}|PCCapture|0|")
        log_file.close()
        return True
    except Exception:
        return False
...
try:
    ...
    setting_file.close()
    if len(setting) != 5 or min(setting) <= 0:
        raise Exception
...
#    cv2.imshow('ADS',frame_ABS)
    ...
    if not numpy.array_equal(now_setting, setting):
        for i in range(0, 5):
