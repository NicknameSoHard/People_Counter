...
w = cap.get(3)
h = cap.get(4)
min_areaTH = (h*w) / 40 # Делим высоту и длину кадра на выбранный коэфициент. Его мы подбираем из соображений наилучшего обнаружения
...
#    fgmask = fgbg.apply(grayABS) #Применяем вычитание фона субстрактором
    ...
    try:
        ...
        ret,imBin= cv2.threshold(fgmask, 30, 255,cv2.THRESH_BINARY)
